...
void template4() {
        ...
        FILE *file1 = fopen("n3.pgm", "r"); // input file read
	...
	// creat result image;
        XX = Xvalues2[0] - Xvalues1[0];

        FILE *image = fopen("result_image.pgm", "w");
        fprintf(image, "%c%c\n%d %d\n%d\n", M, N, XX, YY, MAX);

        for(y= top_value; y<bottom_value+1; y++) {
        	for(x=Xvalues1[0]; x<Xvalues2[0]; x++) {
        ...
        FILE *file2 = fopen("number_templates.h", "a+"); // write file
        fprintf(file2, "int T_0[30][30] = { \n");
        for(y=0; y<YY; y++) {
        	for(x=0; x<XX; x++) {
